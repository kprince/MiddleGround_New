...
namespace MiddleGround.UI
{
    public class MG_PopPanel_DiceReward : MG_UIBase
    {
        ...
        IEnumerator AutoRotateCardLight()
        {
            ...
            {
                yield return null;
                trans_CardLight.Rotate(0, 0, -Time.deltaTime * 3f);
            }
        }
        IEnumerator AutoScaleCardLight()
        {
            ...
            {
                yield return null;
                trans_CashLight.localScale += isUp ? new Vector3(Time.deltaTime / 2, Time.deltaTime / 2) : -new Vector3(Time.deltaTime / 2, Time.deltaTime / 2);
        ...
        IEnumerator DelayShow(Image btn)
        {
            ...
            {
                yield return null;
                btn.color += new Color(0, 0, 0, 0.05f);
                if (btn.color.a >= 0.9f)
                {
